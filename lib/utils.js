...
ALTER TABLE menu_meta ADD COLUMN IF NOT EXISTS pdf_sha256 TEXT;
...
// Flat {category, position, item_gr, item_en} rows, shared by the
// string-built SQL below and by runWatch's parameterized insert.
export function flattenBreakfastRows(bGr, bEn) {
  const rows = [];
  for (const category of Object.keys(bGr)) {
    bGr[category].forEach((gr, i) => {
      rows.push({ category, position: i + 1, item_gr: gr, item_en: bEn[category][i] });
    });
  }
  return rows;
}
...
export function renderBreakfastSql(bGr, bEn) {
  const rows = flattenBreakfastRows(bGr, bEn).map(
    (r) => `('${sqlEscape(r.category)}', ${r.position}, '${sqlEscape(r.item_gr)}', '${sqlEscape(r.item_en)}')`
  );
...
// Flat {week_num, day_name, meal_type, course, position, item_gr, item_en}
// rows, shared by the string-built SQL below, runWatch's parameterized
// insert, and the sanity check on parsed item count before any DB write.
export function flattenMenuItemRows(weeksGr, weeksEn) {
  const rows = [];
      ...
      for (const mealType of ["lunch", "dinner"]) {
        for (const course of ["first", "main", "extra"]) {
          ...
          items.forEach(([gr, en], i) => {
            rows.push({ week_num: weekNum, day_name: day, meal_type: mealType, course, position: i + 1, item_gr: gr, item_en: en });
          });
        }
      }
    }
...
export function renderMenuItemsSql(weeksGr, weeksEn) {
  const rows = flattenMenuItemRows(weeksGr, weeksEn).map(
    (r) =>
      `(${r.week_num}, '${r.day_name}', '${r.meal_type}', '${r.course}', ${r.position}, '${sqlEscape(r.item_gr)}', '${sqlEscape(r.item_en)}')`
  );
  if (!rows.length) return "";
