...
export async function findPdfUrl(pageUrl) {
  ...
  const resolved = new URL(match[2], pageUrl);
  // The path match alone doesn't rule out a link to some other domain that
  // happens to have a wp-content/uploads/....pdf path. Since whatever this
  // resolves to gets downloaded and its content written to the database,
  // require it to actually be on the same host as the page we trust.
  if (resolved.hostname !== new URL(pageUrl).hostname) {
    throw new Error(`PDF link resolved to an unexpected host: ${resolved.hostname}`);
  }
 ...
 * without touching the database. Used for manual review/preview. Includes
 * itemCount so the caller can warn if a parse came back suspiciously empty
 * (the SQL would still truncate menu_items even with nothing to replace it).
 */
export async function generateSqlPreview() {
  const pdfUrl = await findPdfUrl(PAGE_URL);
  const data = await downloadPdf(pdfUrl);
  const { sql, itemCount } = await convertPdfBytesToSql(data, pdfUrl);
  return { pdfUrl, sql, itemCount, lowItemCount: itemCount < MIN_MENU_ITEMS };
}
