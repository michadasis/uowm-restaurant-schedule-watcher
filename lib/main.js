...
import { parseWeekFromPage, buildSql, flattenMenuItemRows } from "./utils.js";
...
// Returns the SQL alongside the parsed weeks and a menu-item count, so
// callers that write to a database (unlike the CLI, which just wants the
// SQL) can sanity-check the parse before doing anything destructive.
export async function convertPdfBytesToSql(data, sourcePdfUrl = null) {
  ...
  const sql = buildSql(weeksGr, weeksEn, weeksGr.length, sourcePdfUrl, BREAKFAST_GR, BREAKFAST_EN);
  const itemCount = flattenMenuItemRows(weeksGr, weeksEn).length;
  return { sql, itemCount, weeksGr, weeksEn };
}
...
async function main() {
  ...
  const { sql: sqlContent } = await convertPdfToSql(pdfPath, sourcePdfUrl || null);
